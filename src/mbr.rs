...
impl MbrPartition {
    ...
    fn new(partition_buffer: [u8; 16]) -> Self {
        ...
        let chs_start: [u8; 3] = partition_buffer[1..4].try_into().unwrap();
        let chs_end: [u8; 3] = partition_buffer[5..8].try_into().unwrap();
        ...
        let num_sectors = u32::from_ne_bytes(partition_buffer[12..16].try_into().unwrap());
